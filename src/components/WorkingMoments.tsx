...
export function WorkingMoments() {
  ...
  return (
    <section id="gallery" className="py-24 md:py-32 bg-transparent relative z-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 auto-rows-[200px] grid-flow-dense">
          {images.map((img, index) => (
            <motion.div
              ...
              className={cn(
                "relative group overflow-hidden rounded-2xl cursor-pointer glass-panel",
                img.span
              )}
            ...
            >
              ...
              <div className="absolute inset-0 bg-primary/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                <div className="bg-white/20 backdrop-blur-md p-3 rounded-full text-neutral-white transform scale-50 group-hover:scale-100 transition-transform duration-300">
                  <ZoomIn size={24} />
                </div>
              </div>
            </motion.div>
      ...
      <AnimatePresence>
        {selectedImage && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[100] flex items-center justify-center bg-primary/90 p-4 backdrop-blur-xl"
            onClick={() => setSelectedImage(null)}
          >
            <button
              className="absolute top-6 right-6 text-neutral-white hover:text-secondary transition-colors bg-white/10 p-2 rounded-full"
              onClick={() => setSelectedImage(null)}
            >
              <X size={32} />
            </button>
